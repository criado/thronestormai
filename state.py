...
class Card:
  ...
  def __str__(self):
    return self.suits[self.suit] + self.values[self.value];
...
class state:
  ...
  def all_neighbours(self): # Lists all neighbours of a state. Depending on the action, the phase of a turn may change.
    ...
    if self.phase==1:
      for i in range(4):
        if not self.decks[i] and self.reliquary[i]==None:
    ...
    for suit in range(4):
      if self.relicary[suit]==None: continue

      s2=deepcopy(self)
      c= s2.relicary[suit]; s2.relicary[suit]=None; s2.phase=1
      ...
      if s2.inventories[0][suit]==None:
        ...
        for x,y in s2.next_empty_cells():
          ...
          s3=deepcopy(s2)
          s3.inventories[0][suit],c=c,s3.inventories[0][suit]
          s3.realm[x][y]=c
          neighbours.append(s3)
